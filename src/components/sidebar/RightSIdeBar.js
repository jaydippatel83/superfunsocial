"use client";
import React, { useEffect, useState } from "react";
import RecentList from "../animation/RecentList";
import axios from "axios";
import SuggestedUsers from "./SuggestedUsers";
import { useNeynarContext } from "@neynar/react";

const RightSIdeBar = () => {
  const { user } = useNeynarContext();
  const [userData, setUserData] = useState([]);
  const [trendingChannels, setTrendingChannels] = useState([]);

  const getActiveUser = async (id) => {
    const headers = {
      accept: "application/json",
      api_key: process.env.NEXT_PUBLIC_NEYNAR_API_KEY,
    };
    const url = `https://api.neynar.com/v2/farcaster/user/active?limit=5`;
    try {
      const response = await axios.get(url, {
        headers,
      });
      const data = response.data?.users;
      setUserData(data);
    } catch (error) {
      console.log(error);
    }
  };

  const getTrendingChannels = async (id) => {
    const headers = {
      accept: "application/json",
      api_key: process.env.NEXT_PUBLIC_NEYNAR_API_KEY,
    };
    const url = `https://api.neynar.com/v2/farcaster/channel/trending?time_window=7d&limit=10`;
    try {
      const response = await axios.get(url, {
        headers,
      });
      const data = response.data?.channels;
      setTrendingChannels(data);
    } catch (error) {
      console.log(error);
    }
  };

  useEffect(() => {
    getActiveUser();
    getTrendingChannels();
  }, [user]);

  return (
    <div className=" lg:max-w-[400px] sm:max-w-[580px]  mx-auto flex-1   space-y-3">
      <div
        className="lg:space-y-4 sm:space-x-2 lg:pb-8  sm:grid-cols-2 max-lg:gap-6" 
      >
        <div className="box p-5 px-6 mb-3">
          <div className="flex items-baseline justify-between text-black dark:text-white">
            <h3 className="font-bold text-base">Trending Channels 📣</h3>
            <a href="#" className="text-sm text-blue-500">
              See all
            </a>
          </div>
          <RecentList trendingChannels={trendingChannels} />
        </div>
        <SuggestedUsers userData={userData} />
      </div>
    </div>
  );
};

export default RightSIdeBar;
