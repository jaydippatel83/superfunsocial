...
const Profile = () => {
  ...
  return (
    <div className="relative">
      <div className="rounded-full relative bg-secondery cursor-pointer shrink-0" onClick={toggleDropdown}>
      ...
      {isDropdownOpen && (
        <div ref={dropdownRef} className="absolute top-full right-0 mt-2 bg-white rounded-lg drop-shadow-xl dark:bg-slate-700 w-64 border2 z-10">
          ...
          <nav className="p-2 text-sm text-black font-normal dark:text-white">
            ...
            <button type="button" className="w-full" onClick={toggleNightMode}>
              <div className="flex items-center gap-2.5 hover:bg-secondery p-2 px-2.5 rounded-md dark:hover:bg-white/10">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
                ...
                <span className="bg-slate-200/40 ml-auto p-0.5 rounded-full w-9 dark:hover:bg-white/20">
                  <span className={`bg-white block h-4 relative rounded-full shadow-md w-2 w-4 ${isNightMode ? 'dark:bg-blue-600' : ''}`}></span>
                </span>
              </div>
            </button>
